...
import { transformPrompt } from "./transform-request-v2";
import { AIChatRequestBody } from "./types";

export const transformRequest = (
  request: AIChatRequestBody,
  prompt: FullPrompt,
  options: Omit<OpenAI.ChatCompletion.ChatCompletionRequest, "messages">
  ...
  const chatMessages: { role: "assistant" | "user"; content: string }[] = request.messages.map(
    (item) => {
